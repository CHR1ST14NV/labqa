...
export function OrderForm() {
  const { draft, updateDraft, calculateQuote } = useDemoStore();
  ...
  return (
    <Panel>
      ...
      <div className="space-y-4">
        ...
        <Field label="Precio (Q)">
          <Input
            ...
            onChange={(event) => updateDraft({ subtotal: Number(event.target.value) || 0 })}
          />
        </Field>
      ...
      <Button size="lg" className="mt-6 w-full gap-2" onClick={calculateQuote} disabled={draft.subtotal <= 0}>
